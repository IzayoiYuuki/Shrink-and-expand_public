...
OutNames = './Figure/' + MatNames + '_ISI_fix.pdf';
...
dlmwrite('./Figure/Data_ISI_fix.txt', date, '-append', 'delimiter', '', 'precision', 4);

for fileNo = 1 : 11

    disp(MatNames(fileNo));
    dlmwrite('./Figure/Data_ISI_fix.txt', fileNo, '-append', 'delimiter', '', 'precision', 4);
    ...
    tic;
    [~, aaa, iter, res, ~] = myInvSubspaceIteration(A, X, nev, tol, Maxiter, SEconfig);
    timeL(fileNo, 1) = toc;
    iterL(fileNo, 1) = iter;
    ...
    % save data
    dlmwrite('./Figure/Data_ISI_fix.txt', timeL(fileNo, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_ISI_fix.txt', iterL(fileNo, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_ISI_fix.txt', resL{fileNo}(1, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_ISI_fix.txt', resL{fileNo}(2, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_ISI_fix.txt', shrinklistL(fileNo, :), '-append', 'delimiter', ',', 'precision', 4);
