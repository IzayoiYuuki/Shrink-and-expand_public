...
OutNames = './Figure/' + MatNames + '_LOBPCG_slopek.pdf';
...
dlmwrite('./Figure/Data_LOBPCG_slopek.txt', date, '-append', 'delimiter', '', 'precision', 4);

for fileNo = 1 : 12

    disp(MatNames(fileNo));
    dlmwrite('./Figure/Data_LOBPCG_slopek.txt', fileNo, '-append', 'delimiter', '', 'precision', 4);
    ...
    tic;
    [~, ~, iter, res, ~] = myLOBPCG(A, B, X, Myprec, nev, tol, Maxiter, SEconfig);
    timeL(fileNo, 1) = toc;
    iterL(fileNo, 1) = iter;
    ...
    tic;
    [~, ~, iter, res, SEconfig.shrinklist] = myLOBPCG(A, B, X, Myprec, nev, tol, Maxiter, SEconfig);
    timeL(fileNo, 2) = toc;
    iterL(fileNo, 2) = iter;
    ...
    % save data
    dlmwrite('./Figure/Data_LOBPCG_slopek.txt', timeL(fileNo, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_LOBPCG_slopek.txt', iterL(fileNo, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_LOBPCG_slopek.txt', resL{fileNo}(1, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_LOBPCG_slopek.txt', resL{fileNo}(2, :), '-append', 'delimiter', ',', 'precision', 4);
    dlmwrite('./Figure/Data_LOBPCG_slopek.txt', shrinklistL(fileNo, :), '-append', 'delimiter', ',', 'precision', 4);
